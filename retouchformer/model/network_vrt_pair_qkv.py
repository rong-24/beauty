...
class WindowAttention(nn.Module):
    ...
    def forward(self, x, mask = None, mask_windows = None, mask_res_windows = None):
        """ Forward function.
        ...
        k  = self.k_self(x).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        v  = self.v_self(x * mask_res_windows).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
        ...
        if self.mut_attn:
            ...
            k = self.k_mut(x + self.position_bias.repeat(1, 2, 1)).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)
            v = self.v_mut(x * mask_res_windows + self.position_bias.repeat(1, 2, 1)).reshape(B_, N, 1, self.num_heads, C // self.num_heads).permute(2, 0, 3, 1, 4)            
...
class TMSA(nn.Module):
    ...
    def forward_part1(self, x, mask_matrix, mask):
        B, D, H, W, C = x.shape
        ...
        attn_windows = self.attn(x_windows, mask=attn_mask, mask_windows=mask_windows, mask_res_windows = mask_res_windows)  # B*nW, Wd*Wh*Ww, C
    ...
    def forward(self, x, mask_matrix, mask):
        """ Forward function.
        ...
        # else:
        x = x + self.forward_part1(x, mask_matrix, mask)
...
class Stage(nn.Module):
    ...
    def forward(self, x, mask):   
        ...
        x = self.linear1(self.residual_group1(x, mask).transpose(1, 4)).transpose(1, 4) + x
        x = self.linear2(self.residual_group2(x, mask).transpose(1, 4)).transpose(1, 4) + x
        return x
